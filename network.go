...
func fetchPage(query, sorting, ratio, atleast string, page int) ([]Wallpaper, int) {
	...
	// Добавляем API-ключ ТОЛЬКО если он существует и не является строкой-заглушкой
	...
	// === ОТЛАДКА: выводим в консоль запрос ===
	fmt.Println("[СЕТЬ] Ищу обои:", url)

	resp, err := httpClient.Get(url)
	if err != nil {
		fmt.Println("[ОШИБКА] Сетевая ошибка:", err)
		return nil, 0
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		fmt.Printf("[ОШИБКА] Сервер Wallhaven вернул статус-код: %d (проверьте API-ключ)\n", resp.StatusCode)
		return nil, 0
	}

	var r APIResponse
	if err := json.NewDecoder(resp.Body).Decode(&r); err != nil {
		fmt.Println("[ОШИБКА] Ошибка разбора ответа сервера:", err)
		return nil, 0
	}

	// Если сервер вернул пустой результат
	if len(r.Data) == 0 {
		fmt.Println("[ИНФО] По вашему запросу ничего не найдено.")
	}
...
// === НОВАЯ ФУНКЦИЯ ДЛЯ ЗАГРУЗКИ ТЕГОВ ===
func fetchTags(id string) []string {
	...
	// Временная структура для разбора ответа API
	var r struct {
		Data struct {
			Tags []struct {
